//! Thread-safe pool of per-frame CUDA stream slots (one CUstream + buffers each).
...
pub fn Pool(comptime Stream: type, comptime Owner: type) type {
    return struct {
        ...
        /// Reserve free-list capacity. Must succeed before prewarm (uses appendAssumeCapacity).
        pub fn prime(self: *Self, owner: *Owner, hint: usize) !void {
            self.owner = owner;
            try self.free.ensureTotalCapacity(allocator, hint);
            try self.all.ensureTotalCapacity(allocator, hint);
        }

        /// Create up to `n` streams with the CUDA context current. Stops early on OOM
        /// once at least one stream exists.
        pub fn prewarm(self: *Self, n: usize) !void {
            ...
            while (i < n) : (i += 1) {
                ...
                s.init(self.owner) catch |err| {
                    allocator.destroy(s);
                    if (self.all.items.len > 0 and (err == error.OutOfDeviceMemory or err == error.OutOfMemory)) break;
                    return err;
                };
        ...
        /// Caller must hold the CUDA context (Stream.deinit frees device resources).

...
void fused(
    ...
    constexpr int tiles_per_warp = static_cast<int>(TILES_PER_WARP);
    __shared__ float2 shared_transpose_buffer[warps_per_block * tiles_per_warp * block_size * transpose_stride];

    int warp_id = threadIdx.x / warp_size;
#if TILES_PER_WARP == 2
    int sub = (threadIdx.x % warp_size) / block_size;
    int lane_id = threadIdx.x % block_size;
    const unsigned mask_shift = sub * block_size;
#else
    constexpr int sub = 0;
    int lane_id = threadIdx.x % warp_size;
    constexpr unsigned mask_shift = 0;
#endif
    auto transpose_buffer = &shared_transpose_buffer[(warp_id * tiles_per_warp + sub) * block_size * transpose_stride];

    for (int block_id = (blockIdx.x * WARPS_PER_BLOCK + warp_id) * tiles_per_warp + sub; block_id < num_blocks; block_id += gridDim.x * WARPS_PER_BLOCK * tiles_per_warp) {
        int ix = block_id % horizontal_num;
        int iy = block_id / horizontal_num;

        if (lane_id < block_size) {
            const unsigned active_mask = ((1u << block_size) - 1) << mask_shift;
            ...
            for (int i = 0; i < 2 * radius + 1; i++) {
                ...
                if (lane_id < block_size / 2 + 1) {
                    ...
                    __syncwarp(((1u << (block_size / 2 + 1)) - 1) << mask_shift);
            ...
            if (lane_id < block_size / 2 + 1) {
                ...
                gf = __shfl_sync(((1u << (block_size / 2 + 1)) - 1) << mask_shift, gf, mask_shift);
#endif
                #pragma unroll
                for (int i = 0; i < 2 * radius + 1; i++) {
                    #pragma unroll
            ...
            __syncwarp(active_mask);
            auto local_dst = &dstp[block_id * block_size * block_size];
            #pragma unroll
            for (int j = 0; j < block_size; j++) {
...
void col2im(
    ...
    for (int i = i1; i <= i2; i++) {
        ...
        for (int j = j1; j <= j2; j++) {
            ...
            auto src_offset = ((i * horizontal_num + j) * block_size + offset_y) * block_size + offset_x;
            auto window_offset = (radius * block_size + offset_y) * block_size + offset_x;
            sum += src[src_offset] * window[window_offset];
        }
    }

    dst[(y - pad_y) * dst_stride + (x - pad_x)] = from_float(sum);
}

// Single-fold mirror; valid only when pad <= dim-1.
extern "C"
__global__
void reflect_pad(
    TYPE * __restrict__ dst,
    ...
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= pad_width || y >= pad_height) {
        return ;
    }
    int offset_x = (pad_width - width) / 2;
    int offset_y = (pad_height - height) / 2;
    int sx = x < offset_x ? offset_x - x : (x >= offset_x + width ? offset_x + 2 * width - 2 - x : x - offset_x);
    int sy = y < offset_y ? offset_y - y : (y >= offset_y + height ? offset_y + 2 * height - 2 - y : y - offset_y);
    dst[y * pad_width + x] = src[sy * src_stride + sx];
}

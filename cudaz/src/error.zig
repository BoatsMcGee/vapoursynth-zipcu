...
// CUDA headers report status as unsigned on some platforms and signed (enum/c_int) on others (e.g. MSVC).
pub fn fromCudaErrorCode(error_code: anytype) CudaError.Error!void {
    const code: u32 = @intCast(error_code);
    if (code == 0) return;
    const val = CudaError.from_error_code(code).?;
    return val;
}
pub fn fromNvrtcErrorCode(error_code: anytype) NvrtcError.Error!void {
    const code: u32 = @intCast(error_code);
    if (code == 0) return;
    const val = NvrtcError.from_error_code(code).?;
    return val;
}

pub fn fromCurandErrorCode(error_code: anytype) CurandError.Error!void {
    const code: u32 = @intCast(error_code);
    if (code == 0) return;
    const val = CurandError.from_error_code(code).?;
    return val;
}
